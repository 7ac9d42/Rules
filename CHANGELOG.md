...
- 恢复三/四机场配置全部 32 个可见分组的用途图标，已有资源改用仓库链接；补充可见图标校验，分流与回退策略不变。

- `configfull_new.yaml`采用三机场单文件设计，四机场家宽模板独立为`cinfigfull_new_4.yaml`；`configfull.bak.yaml`保留远程原版。
- 业务select独立保存手选；四层政策决定默认与自动回退，CF/GitHub/通用探针在对应候选中择优。机场名称1默认日本→香港→新加坡→美国。
- 保留严格媒体独立控制，聚合等价普通业务；纯下载独立并允许同机场跨地区择优，移除load-balance。
- AI默认日本手选，Tunnel默认DIRECT，均允许用户主动覆盖机场、地区、共享家宽或实际节点；GLOBAL支持临时单节点全局代理。
- 重启保留有效手选；明确国内直连优先。Tunnel端点7844专用分流与两region真实IP解析独立处理。
- 配置块式排版，删除无效定义和重复继承参数；校验入口同步覆盖三/四机场配置。固定内核验收更新为v1.19.30，旧v1.19.19不支持rematch。

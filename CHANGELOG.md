...
- 重排主配置的分流优先级：广告、自定义覆盖及 Apple/B 站等跨区策略之后，先以 `cn_domain` 保障国内域名直连，再处理常规代理服务；具体服务规则之后新增可解析的 `cn_ip` 安全网，并将 Cloudflare、GFW、`geolocation-!cn` 与最终代理兜底统一后移，减少国内域名被宽泛代理集合提前截获。

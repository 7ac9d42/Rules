...
- 自动链路先在同机场换地区，再跨机场。普通机场名称1按日本 → 香港 → 新加坡 → 美国；普通机场名称3按日本 → 新加坡 → 香港 → 美国。
- 纯下载自动链路在同机场内按**日本 → 新加坡 → 美国 → 香港**回退，每个地区内再自动选点；机场名称4在这些地区之后保留其他地区兜底。默认仍按成本优先的机场顺序，因此机场名称3的香港可用时，仍先于机场名称1的日本；显式手选地区或节点时遵循用户选择。下载池继续使用各机场原有的倍率、CTCU、MITM 等筛选规则，内部池隐藏，可见业务组和选项不变。
...
纯下载的地区顺序依据已确认的使用偏好设定；香港即使探针延迟更低，也不会抢过可用的日本。官方 [`url-test`](https://github.com/MetaCubeX/mihomo/blob/v1.19.30/adapter/outboundgroup/urltest.go) 不提供按带宽给地区加权的字段，`tolerance` 是切换容差；这里用 [`fallback`](https://wiki.metacubex.one/config/proxy-groups/fallback/) 实现明确优先级。日本节点若仍通过探针但带宽下降，不会因此自动转往美国或香港；需结合实际下载表现手选或调整优先级。
